...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{eqnarray}
			...
			&=& \int q(\bm{Z}) \left( \ln p(\bm{Z} | \bm{X}) + \ln p(\bm{X}) \right) d\bm{Z} + H[q] \\
			&=& \int q(\bm{Z}) \ln p(\bm{Z} | \bm{X}) d\bm{Z} + \ln p(\bm{X}) + H[q] \\
			&=& \mathbb{E}_{\bm{Z} \sim q(\bm{Z})} \left[ \ln p(\bm{Z} | \bm{X}) \right] + \ln p(\bm{X}) + H[q]
		\end{eqnarray}
		
		\item 分布に含まれるパラメータを明示的に記述する
		\item $p(\bm{Z} | \bm{X}, \bm{\theta})$と、$p(\bm{Z} | \bm{\theta})$をモデリングする
		\item このとき、$p(\bm{X}, \bm{Z} | \bm{\theta}) = p(\bm{Z} | \bm{X}, \bm{\theta}) p(\bm{X} | \bm{\theta})$と書ける
		\item 従って、下界$\mathcal{L}(q)$は次のようになる
		\begin{equation}
			\mathcal{L}(q, \bm{\theta}) = \mathbb{E}_{\bm{Z}} \left[ \ln p(\bm{Z} | \bm{X}, \bm{\theta}) \right] + \ln p(\bm{X} | \bm{\theta}) + H[q]
		\end{equation}
		
		\item 下界$\mathcal{L}(q, \bm{\theta})$を\color{red}パラメータ$\bm{\theta}$について\normalcolor 増加させるとする
		\item これは、$\mathbb{E}_{\bm{Z}} \left[ \ln p(\bm{Z} | \bm{X}, \bm{\theta}) \right]$を、\color{red}$q$を固定しつつ\normalcolor 、$\bm{\theta}$について増加させることに繋がる
		\newline
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item データ$\bm{X}$から潜在変数$\bm{Z}$への写像は、モデル($p(\bm{X} | \bm{Z} | \bm{\theta}), p(\bm{Z} | \bm{\theta})$の選択に依存し、更に$\bm{\theta}$の変化に伴って変わり続ける
		\newline
